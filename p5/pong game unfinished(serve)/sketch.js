...
function keyPressed(){ //every time you push a key, make a new ball from the ball class and add it to the balls array
  if (key==='b') {
    let  b = new Ball(625, 200, 5*(serveDirection),0.5, random(-2,2), false, random(1,3));//MAKE A SERVING X VALUE
    balls.push(b);
...
class Ball {
	...
	constructor(x,y, speedx, speedy, randomSpeed, scored,server){
		this.x = x;
    this.y = y;
    this.speedx = speedx;
    this.speedy = speedy;
	...
	drawBall(){
        	...
        	strokeWeight(1);
    		fill("red");
		ellipse(this.x+this.serveLocation,this.y,10,10);
	}
